...
def test_variant_outcomes_not_co_emitted_by_columnar(variant):
    """REQ_144: variant_outcomes is a derived table, not a columnar-pass co-emission.

    The old summary-JSON flatten (with its ``summary_json`` carrier) is gone — the
    columnar pass no longer produces ``variant_outcomes``; it is now a pure
    DerivedTableSpec joining the stable outcome clusters, materialized by the derived
    pass. Its content is covered by ``test_derived_materialize`` and the assembler's
    baseline parity.
    """
    materialize_variant_columnar(variant)
