...
**Status:** Completed (2026-05-27) — *Merged to `develop`. All view re-points shipped and validated on two models: canon (p113/s999/ds598; 64 unique views pass, 0 fail, peak RSS 937 MB) and p101/s999/ds598 (re-analyzed — views confirmed not-loading-before / loading-after the new-analyzer refresh). Phase B composition object restores the defused Fourier-alignment trace. Two memory fixes landed (pipeline reference release + selective field loading — the latter resolved an OOM that was crashing WSL during page smoke-tests). family.json analyzer lists cleaned across all three families. Full suite green (1643 pass / 27 skip), dashboard 45 pass. Hands the analyzer retirements to REQ_102; the analysis-pipeline memory climb observed during re-analysis is a distinct surface tracked under REQ_128.*
...
- **Memory investigation (2026-05-27 during Phase A.2e)**: user flagged high memory; targeted review of the three new analyzers (`weight_basis_projection`, `activation_basis_projection`, `weight_spectra`) and the `centroid_fourier_alignment` analyzer found **no leaks in the analyzers themselves** — all compose functions and projection primitives detach/cpu/numpy at boundaries, so result dicts hold no torch references. The one defensive fix applied lives in the pipeline: `_run_single_epoch` now nulls `inputs`/`result`/`summary` before `del model, cache, ...` so the last analyzer iteration's `ResolvedInputs` references don't keep model + cache alive past the `empty_cache()` call. Other potential memory sources (CUDA allocator behavior, ArtifactLoader caching, summary_collectors growth across epochs) weren't profiled — flag for follow-up if memory pressure persists after this fix.
...
- **Removal of experimental `dataview_universal.py` / `dataview_catalog.py`.** If touched during Phase B, removal is preferred over extension. Otherwise stays as-is until a future cleanup REQ revisits. (Phase B did not touch them — it added a clean `views/dataviews.py` with the single `RepresentationGeometryTimeseries` composition object.)

### REQ_102 handoff items (surfaced during Phase A)

- **Export registry Fourier entries.** `visualization/export.py`'s `_VISUALIZATION_REGISTRY` is a second consumer surface parallel to the view catalog, but `export_variant_visualization` has **no consumers** anywhere in src/apps/scripts. Its `effective_dimensionality` entries were re-pointed to `weight_spectra` (clean 1-to-1). Its Fourier entries (`dominant_frequencies`, `neuron_freq_norm`, `attention_freq`) still reference retiring analyzers — re-pointing them cleanly would require replicating the view-layer legacy adapters, so they're left for REQ_102 to remove alongside the analyzer code. Consider whether the whole unused export API should be retired.
- **Coarseness REQ_102 gate needs canon data.** `test_req102_coarseness_recoverable_from_activation_basis_projection` (the blob-vs-plaid preservation gate) skips when canon lacks the legacy `coarseness` artifact — which it currently does. To record that gate's evidence, canon must be re-analyzed with the `coarseness` analyzer present, or the gate re-expressed against a variant that has it.
- **Transition renderers.** `dominant_frequencies.py`, `attention_fourier.py`, `attention_freq.py`, `neuron_freq_clusters.py` stay in place feeding the re-pointed views via legacy-shape adapters. REQ_102 deletes the files outright when it retires the underlying analyzers.
